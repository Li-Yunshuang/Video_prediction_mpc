...
def build_tfrecord_input(conf, training=True):
    ...
    load_indx = range(0, 45, conf['skip_frame'])
    load_indx = load_indx[:conf['sequence_length']]  #load sequence (total )
    print 'using frame sequence: ', load_indx

    # for each frame in one sequence
    for i in load_indx:
...
if __name__ == '__main__':
    ...
    conf['sequence_length']= 45      # 'sequence length, including context frames.'
    conf['use_state'] = True
    conf['batch_size']= 2 #32
    conf['visualize']= True
    conf['single_view'] = ''
