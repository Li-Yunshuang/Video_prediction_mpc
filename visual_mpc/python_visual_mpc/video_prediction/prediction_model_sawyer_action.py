...
class Prediction_Model(object):
    ...
    def __init__(self,
        ...
        #self.img_height, self.img_width, self.color_channels = [int(i) for i in images[0].get_shape()[0:3]]  
        #self.batch_size = 1
    ...
    def build(self):
        ...
        #batch_size = 1
        #img_height, img_width, color_channels = self.images[0].get_shape()[0:3]
...
# ###################################################### Sampling different actions
                cost_frame = []   #n*1
                action_frame = []  #n*32*9
                for iter in range(40):
                    print iter
                    #  tf.reset_default_graph()
                    #  random yige action [32,9]
                    action = np.random.rand(1,2)
                    delta = 0.5/30 * math.pi *2
                    action = action * delta - 0.5/30 * math.pi
                    
                    act_2 = np.zeros((1,2))
                    action = np.concatenate([action, act_2],axis=0)
                    ...
                    #self.gen_images.append(output) ###################get gen_images
                    ...
                    #print output
                    #print self.gen_images
                    image_cost_mse = mean_squared_error(self.images[t+1], output)
                    ...
                    if current_state != None:
                        current_state = slim.layers.fully_connected(
                            ...
                            scope='state_pred',
                            activation_fn=None)
                    state_cost_mse = mean_squared_error(self.states[t+1], current_state)

                    total_cost = image_cost_mse + state_cost_mse
                    
                    cost_frame.append(total_cost)
                ...
                self.ranaction_all.append(action_frame)
                self.cost_all.append(cost_frame)
